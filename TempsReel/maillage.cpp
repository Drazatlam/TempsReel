...
std::vector<float> Maillage::points(){
	std::vector<float> resu;
	resu.reserve(topo.size()*3);
	for(int i=0;i<topo.size();++i){
		resu.push_back(geom[topo[i]].x);
		resu.push_back(geom[topo[i]].y);
		resu.push_back(geom[topo[i]].z);
	}
	return resu;
}
		
std::vector<float> Maillage::normals(){
	std::vector<float> resu;
	resu.reserve(normalIds.size()*3);
	for(int i=0;i<normalIds.size();++i){
		resu.push_back(normales[normalIds[i]].x);
		resu.push_back(normales[normalIds[i]].y);
		resu.push_back(normales[normalIds[i]].z);
	}
	return resu;
}
